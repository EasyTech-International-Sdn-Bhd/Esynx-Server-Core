...
func (r *CmsInvoiceRepository) Update(invoice *entities.CmsInvoice) error {
	_, err := r.db.Where("invoice_code = ?", invoice.InvoiceCode).Omit("invoice_code", "ref_no").Update(invoice)
	if err != nil {
		return err
	}
...
func (r *CmsInvoiceRepository) UpdateMany(invoices []*entities.CmsInvoice) error {
	for _, inv := range invoices {
		_, err := r.db.Where("invoice_code = ?", inv.InvoiceCode).Omit("invoice_code", "ref_no").Update(inv)
		if err != nil {
			return err
		}
	}
