...
func (r *CmsDebitNoteRepository) Update(debitNote *entities.CmsDebitnote) error {
	_, err := r.db.Where("dn_code = ?", debitNote.DnCode).Omit("dn_code").Update(debitNote)
	if err != nil {
		return err
	}
...
func (r *CmsDebitNoteRepository) Delete(debitNote *entities.CmsDebitnote) error {
	...
	_, err := r.db.Where("dn_code = ?", debitNote.DnCode).Delete(&entities.CmsDebitnote{})
	if err == nil {
...
func (r *CmsDebitNoteRepository) UpdateMany(debitNotes []*entities.CmsDebitnote) error {
	for _, dn := range debitNotes {
		_, err := r.db.Where("dn_code = ?", dn.DnCode).Omit("dn_code").Update(dn)
		if err != nil {
			return err
		}
	}
...
func (r *CmsDebitNoteRepository) DeleteMany(debitNotes []*entities.CmsDebitnote) error {
	...
	_, err := r.db.In("dn_code", ids).Delete(&entities.CmsDebitnote{})
	if err != nil {
		return err
	}
...
func (r *CmsDebitNoteRepository) DeleteByAny(predicate *builder.Builder) ([]*entities.CmsDebitnote, error) {
	var t entities.CmsDebitnote
